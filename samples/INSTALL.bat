@ECHO OFF
set skript=%~dp0\Install-ADSK.ps1
...
REM powershell.exe -ExecutionPolicy Bypass -File %skript% -WIM %wim% -Mode "Install" -Path %wimpath% -Files "INV_VLT" -Logging -Purge

REM If multiple Collection files should be installed, the following line can be used:
REM powershell.exe -ExecutionPolicy Bypass -File %skript% -WIM %wim% -Mode "Install" -Path %wimpath% -Files "INV_ONLY","VAULT" -Logging -Purge
